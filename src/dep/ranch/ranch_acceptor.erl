...
loop(LSocket, Transport, Logger, ConnsSup, MonitorRef) ->
	io:format("LSocket = ~w, Transport = ~w, Logger = ~w, ConnsSup = ~w, MonitorRef = ~w~n",[LSocket, Transport, Logger, ConnsSup, MonitorRef]),
	_ = case Transport:accept(LSocket, infinity) of
		{ok, CSocket} ->
			case Transport:controlling_process(CSocket, ConnsSup) of
				ok ->
					io:format("11111111111111111111 self =~w ~n",[self()]),
					%% This call will not return until process has been started
					%% AND we are below the maximum number of connections.
					ranch_conns_sup:start_protocol(ConnsSup, MonitorRef,
						CSocket);
				{error, ErrData} ->
					io:format(" 222222222222222 -------------- ErrData = ~w ~n",[ErrData]),
